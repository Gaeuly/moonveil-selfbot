...
let aiConfig = {
    enabled: false,
    prompt: "You are not any AI model, you are a very smart human being and can do everything, if someone asks who you are, you should not answer as any AI model , You are alive, you are smart in all languages, And you understand all human feelings, Who are you? You are you."
};
    ...
    async handleAiResponse(client, message) {
        ...
        if (message.reference && message.reference.messageId) {
            try {
                ...
                if (channel) {
                    const repliedToMessage = await channel.messages.fetch(message.reference.messageId);
                    if (repliedToMessage.author.id === client.user.id) {
                        isReplyToMe = true;
                    }
                }
            } catch (e) {
                // Abaikan jika pesan yang di-reply
            }
        }
